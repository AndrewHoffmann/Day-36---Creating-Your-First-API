    ...
    angular
        ...
        .controller('TableController', function (API) {
			...
			vm.addItem = function(valid){
                if(valid){
                    const kat = Object.assign({},vm.item); // after hit submit, this creates a new Object (person)
                    console.log(kat); 
                                                            // don't need, but shows new object (kat) created in Console Log
                    let addInfo = API.createData(kat);      // we're making addInfo placeholder for ajax call (API.createData)
